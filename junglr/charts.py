...
class PlayerCharts():
    ...
    def firstTower(self, chart, sc_type, sc_list, wlt):
        ...
        for outcomes in wlt:
            ...
            stats = self.summ['Stats'][outcome_name]['teamSummaryData']['firstTower'][sc]
    ...
    def firstDragon(self, chart, sc_type, sc_list, wlt):
        ...
        for outcomes in wlt:
            outcome_name = outcomes['name']
            stats = self.summ['Stats'][outcome_name]['teamSummaryData']['firstDragon'][sc]
            for items in sc_list:
                outcomes['stats'].append(stats[items]['avg'])
        for outcomes in wlt:
            chart.add(outcomes['name'], outcomes['stats'])
    ...
    def goldAdv(self, chart, sc_type, sc_list, wlt):
        ...
        for outcomes in wlt:
            outcome_name = outcomes['name']
            stats = self.summ['Stats'][outcome_name]['participantFrameData']['goldAdvPerFrame']
    ...
    def champs_in_question(self, champ_list):
        if 'all' in champ_list:
            return self.summ['Stats']['total']['gamesPlayed']['championStats'].keys()
        else:
    ...
    def get_sorted_champ_list(self):
        champ_keys = self.summ['Stats']['total']['gamesPlayed']['championStats'].keys()
